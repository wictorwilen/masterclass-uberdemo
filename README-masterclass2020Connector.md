...
2. Give the Connector a name (`Masterclass 2020`), an image and a short and long description. You also need to add your company/Teams Apps website.
3. As *Configuration page for your Connector*  add the following URL, which is used to connect and configure your Connector (`https://masterclassteams.ngrok.io/masterclass2020Connector/config.html`).
4. In the *Valid domains* box enter the Teams Apps website (`https://masterclassteams.ngrok.io`).
5. Choose *Yes* on the *enable actions on your Connector Cards* question.
6. As *Actions URL* add the Connector API endpoint (`https://masterclassteams.ngrok.io/api/connector`).
...
* `./src/masterclass2020Connector/Masterclass2020Connector.ts` - implementation of the Connector that manages registering new subscriptions (`Connect()`) as well as a sample method to send a message to all subscribers (`Ping()`).
* `./src/web/masterclass2020Connector/config.html` - the configuration of the Connector .
* `./src/scripts/masterclass2020ConnectorConfig.tsx` - React component of the configuration page page.
