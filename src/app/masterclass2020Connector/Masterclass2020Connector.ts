 ...
 */
interface IMasterclass2020ConnectorData {
    webhookUrl: string;
    user: string;
    appType: string;
    groupName: string;
...
/**
 * Implementation of the "Masterclass2020ConnectorConnector" Office 365 Connector
 */
@ConnectorDeclaration(
    "/api/connector/connect",
    "/api/connector/ping"
)
@PreventIframe("/masterclass2020Connector/config.html")
export class Masterclass2020Connector implements IConnector {
    ...
    public Ping(req: Request): Array<Promise <void>> {
        ...
        try {
            this.connectors.push("/connectors",
                (this.connectors.getData("/connectors") as IMasterclass2020ConnectorData[]).filter(((c) => {
                    return c.existing;
                })));
        } catch (error) {
        ...
        return (this.connectors.getData("/connectors") as IMasterclass2020ConnectorData[]).map((connector, index) => {
